...
const Page = () => {
  ...
  const getDataDepartment = async () => {
    const res = await api.get("/public/api/master/wards");
  ...
  const getDepartmentById = async (editId) => {
    ...
    const res = await api.get(
      `/public/api/master/wards/${editId}`,
    );
    return res.data;
  };

  const { data: editDepartment, isFetching } = useQuery({
    queryKey: ["wards", editId],
  ...
  const updateDepartmentAPI = async ({ id, name, status }) => {
    const res = await api.put(`/public/api/master/wards/${id}`, {
      name,
      status,
    });
    return res.data;
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (data) => {
    toast.success(data.message || "Ward updated successfully ✅");
  ...
  const addwardsAPI = async ({ name, status }) => {
    const res = await api.post("/public/api/master/wards", {
      name,
      status,
    });
    return res.data;
  ...
  mutationFn: addwardsAPI,
  onSuccess: (data) => {   // 👈 data yahan lo
    toast.success(data.message || "Ward added successfully ✅");
