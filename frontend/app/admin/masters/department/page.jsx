...
const Page = () => {
  ...
  const getDepartmentById = async (editId) => {
    ...
    const res = await api.get(`/public/api/master/departments/${editId}`);
    return res.data;
  };

  const { data: editDepartment, isFetching } = useQuery({
    queryKey: ["department", editId],
  ...
  useEffect(() => {
    if (editDepartment) {
      setName(editDepartment["Department name"]);
  ...
  const updateDepartmentAPI = async ({ id, name, status }) => {
    const res = await api.put(`/public/api/master/departments/${id}`, {
      name,
      status,
    });
    return res.data;
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Department Add Successful!");
  ...
  const addDepartmentAPI = async ({ name, status }) => {
    const res = await api.post("/public/api/master/departments", {
      name,
      status,
    });
    return res.data;
  ...
  const deleteDepartmentAPI = async (id) => {
    const res = await api.delete(`/public/api/master/departments/${id}`);
    return res.data;
  };
  ...
  return (
    <div className="p-6 mt-18">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold">Masters / Departments</h1>
