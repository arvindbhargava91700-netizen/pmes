    ...
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {photos.map((photo) => (
          <div
            key={photo.id}
            className="relative h-40 rounded-xl bg-zinc-50 overflow-hidden group"
          >
            {photo.img ? (
              <>
                {/* IMAGE */}
                ...
                {/* HOVER OVERLAY */}
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-200 z-0" />

                {/* EYE ICON (hover only) */}
                <div className="absolute top-4 right-3 opacity-0 group-hover:opacity-100 transition z-20">
                  <div className="bg-white p-2 rounded-full shadow cursor-pointer">
                    <Eye size={14} className="text-gray-700" />
                  </div>
                </div>

                {/* BOTTOM TEXT (hover only) */}
                <div className="absolute bottom-4 left-4 right-4 opacity-0 group-hover:opacity-100 transition-all duration-300 z-20">
